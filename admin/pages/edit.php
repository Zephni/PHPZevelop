...
<?php
	...
	if(file_exists($tableCfgPath)){ // Check config file exists for this table
		...
		if(count($_POST) > 0){ // If submitted
			...
			if(isset($strToTimeFields) && count($strToTimeFields) > 0){ // Set timestamps for specified fields
			...
			if(isset($fileUploadLocations) && count($fileUploadLocations) > 0){ // Upload files to specified locations
		...
		if(isset($strToTimeFields)){
			if(count((array)$strToTimeFields) > 0){
				foreach($strToTimeFields as $item){ // Check for str to time fields
