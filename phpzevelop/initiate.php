<?php
	...
	/* Check if parameters have been passed, and set them in $_GET
	------------------------------*/
	if(count($PHPZevelop->Get("URLParameters")) > 0)
	{
		foreach(array_reverse($PHPZevelop->Get("URLParameters")) as $key => $value)
			$_GET[$PHPZevelop->CFG->PreParam.$key] = $value;
	}
